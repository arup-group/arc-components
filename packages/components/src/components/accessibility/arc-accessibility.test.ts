...
describe('ArcAccessibility', () => {
  const isOpen = (element: ArcAccessibility) => {
    const drawer: ArcDrawer = element.shadowRoot?.getElementById(
      'drawer',
    ) as ArcDrawer;
  ...
  describe('setters/getters', () => {
    it('activates the accessibility panel when the open state is set', async () => {
      const element: ArcAccessibility = await fixture(
        html`<arc-accessibility open></arc-accessibility>`,
      );
  ...
  describe('methods', () => {
    ...
    beforeEach(async () => {
      ...
      setRootValue(
        `--arc-${stringToHyphenSeparated(testPreference)}-${
          FONT_SIZES['xx-small']
        }`,
        '0.625rem',
      );
      ...
      setRootValue(
        `--arc-${stringToHyphenSeparated(testPreference)}-${FONT_SIZES.small}`,
        '0.875rem',
      );
      setRootValue(
        `--arc-${stringToHyphenSeparated(testPreference)}-${FONT_SIZES.medium}`,
        '1rem',
      );
      setRootValue(
        `--arc-${stringToHyphenSeparated(testPreference)}-${FONT_SIZES.large}`,
        '1.25rem',
      );
      setRootValue(
        `--arc-${stringToHyphenSeparated(testPreference)}-${
      ...
      setRootValue(
        `--arc-${stringToHyphenSeparated(testPreference)}-${
          FONT_SIZES['xxx-large']
        }`,
        '3rem',
      );
      setRootValue(
        `--arc-${stringToHyphenSeparated(testPreference)}-${
          FONT_SIZES['xxxx-large']
        }`,
        '4.5rem',
      );
      setRootValue(
        `--arc-${stringToHyphenSeparated(testPreferenceTwo)}-${
          FONT_SPACING.dense
        }`,
        '1.4',
      );
    ...
    it('should update arc css root variables', async () => {
      ...
      */
      element.updateRootValue(
        testPreference as keyof UserPreferences,
        FONT_SIZES.large,
      );
      expect(
        getRootValue(
    ...
    it('throws an error when trying to update an invalid user preference', async () => {
      ...
      expect(() =>
        element.updateRootValue(
          'testVal' as keyof UserPreferences,
          'someNewValue',
        ),
    ...
    it('throws an error when trying to update a none existing arc css root variable', async () => {
      ...
      expect(() =>
        element.updateRootValue(
          contentPreference as keyof UserPreferences,
          'testVal',
        ),
      ).to.throw(
    ...
    it('should restore all the default arc css root variables', async () => {
      ...
      element.updateRootValue(
        testPreferenceTwo as keyof UserPreferences,
        FONT_SPACING.loose,
      );
  ...
  describe('events', () => {
    ...
    it('should emit arc-accessibility-change when the user preferences change', async () => {
      ...
      const unselectedOptions = [...themeRadioGroup.children].filter(
        (child) =>
          child.tagName === 'ARC-RADIO' && !child.hasAttribute('checked'),
      );
