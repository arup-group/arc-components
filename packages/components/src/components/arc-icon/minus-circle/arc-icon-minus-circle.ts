...
import styles from './arc-icon-minus-circle.styles.js';

/**
 * @cssproperty --icon-color-primary - Set the primary color of the icon.
 * @cssproperty --icon-color-secondary - Set the secondary color of the icon (if supported).
 */
@customElement('arc-icon-minus-circle')
export default class ArcIconMinusCircle extends LitElement {
  /** @internal */
  static tag = 'arc-icon-minus-circle';
  ...
  protected render() {
    return html`
      ...
      >
        ${ArcIconMinusCircle.weightsMap.get(this.weight)}
      </svg>
    `;
  }
}
...
declare global {
  interface HTMLElementTagNameMap {
    'arc-icon-minus-circle': ArcIconMinusCircle;
  }
}
