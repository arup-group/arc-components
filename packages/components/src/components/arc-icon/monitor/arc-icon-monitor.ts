...
import styles from './arc-icon-monitor.styles.js';

/**
 * @cssproperty --icon-color-primary - Set the primary color of the icon.
 * @cssproperty --icon-color-secondary - Set the secondary color of the icon (if supported).
 */
@customElement('arc-icon-monitor')
export default class ArcIconMonitor extends LitElement {
  /** @internal */
  static tag = 'arc-icon-monitor';
  ...
  protected render() {
    return html`
      ...
      >
        ${ArcIconMonitor.weightsMap.get(this.weight)}
      </svg>
    `;
  }
}
...
declare global {
  interface HTMLElementTagNameMap {
    'arc-icon-monitor': ArcIconMonitor;
  }
}
