...
import styles from './arc-icon-wave-triangle.styles.js';

/**
 * @cssproperty --icon-color-primary - Set the primary color of the icon.
 * @cssproperty --icon-color-secondary - Set the secondary color of the icon (if supported).
 */
@customElement('arc-icon-wave-triangle')
export default class ArcIconWaveTriangle extends LitElement {
  /** @internal */
  static tag = 'arc-icon-wave-triangle';
  ...
  protected render() {
    return html`
      ...
      >
        ${ArcIconWaveTriangle.weightsMap.get(this.weight)}
      </svg>
    `;
  }
}
...
declare global {
  interface HTMLElementTagNameMap {
    'arc-icon-wave-triangle': ArcIconWaveTriangle;
  }
}
