...
async function arcPublish() {
  ...
  const packages = Object.values(projects).filter(
    (node) => node.type === 'lib' && node.data.tags?.includes('package'),
  );
  for (const pkg of packages) {
    const distPath = join(
      workspaceRoot,
      pkg.data.targets?.build?.options?.outputPath ??
        `dist/packages/${pkg.name}`,
    );
    ...
    const publishCommand = spawnSync('npm publish --access public', {
