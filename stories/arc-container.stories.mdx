...
The ArcContainer component comes with a default [arc-accessibility](?path=/story/arcaccessibility--default) component,
which allows users to change certain aspects of the website or application to fit their needs.
The accessibility panel can be opened (and closed) by pressing the `a` key anywhere within the page.
Next to that, the [arc-navbar](?path=/story/arcnavbar--default) and [arc-bottombar](?path=/story/arcbottombar--default)
...
Keep in mind that the name of the theme must be either `light` or `dark` as the built-in [arc-accessibility](?path=/story/arcaccessibility--default) component can work with these properties.
